...
export function GetShaderSourceCombinerPassPS() {
    ...
    return (
        ...
        /* glsl */ `));
			...
			//volumeLight *= 0.25;
			float wingsA = clamp(1.0 - volumeLight.r, 0.0, 1.0);
			wingsA *= wingsA;
			/* sceneColor *= wingsA;
			bloom *= MapToRange(wingsA, 0.0, 1.0, 0.5, 1.0); */
			...
			//sceneColor.rgb += volumeLight.rgb;

			//smoke.rgb *= 0.75f * 0.25f;
			vec3 smokeCopy = smoke.rgb;
			smoke.rgb = smoke.rgb * 0.01 + smoke.rgb * bloom.rgb * 2.0;
